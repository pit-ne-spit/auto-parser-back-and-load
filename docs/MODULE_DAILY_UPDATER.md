...
Обрабатывает изменения за вчерашний день и **автоматически запускает нормализацию** после успешного обновления.
...
```bash
# Стандартный запуск (обновление + автоматическая нормализация)
python scripts/daily_update.py
...
После успешного обновления данных скрипт автоматически запускает нормализацию:

1. **Условие запуска:** Нормализация запускается только если:
   - Обновление завершилось без ошибок (`total_errors == 0`), ИЛИ
   - Были обработаны записи (`total_loaded + total_updated + total_removed > 0`)

2. **Что происходит:**
   - Запускается `DataNormalizer` для обработки всех записей с `is_processed = False`
   - Нормализуются новые, обновленные и удаленные записи
   - Результаты сохраняются в таблицу `processed_data`

3. **Обработка ошибок:**
   - Если нормализация завершилась с ошибками, скрипт возвращает код ошибки 1
   - Это позволяет настроить мониторинг и алерты через cron
...
1. **Запуск через Cron:**
   - Настроить ежедневный запуск в определенное время (например, 03:00)
   - Нормализация запускается автоматически после успешного обновления
   - Подробная инструкция: [CRON_SETUP.md](CRON_SETUP.md)

2. **Мониторинг:**
   - Проверять логи на наличие ошибок (`logs/app.log`)
   - Следить за статистикой операций в таблице `operations_log`
   - Скрипт возвращает код 0 при успехе, 1 при ошибке (удобно для мониторинга)

3. **Обработка ошибок:**
   - При ошибках обновления модуль не обновляет `sync_state`
   - Можно безопасно перезапустить модуль - он продолжит с того же места
   - При ошибках нормализации обновление считается успешным, но возвращается код ошибки

4. **Производительность:**
   - Нормализация обрабатывает записи батчами (по умолчанию 200 записей)
   - Можно настроить размер батча через `--normalization-batch-size` или `config.yaml`
